...
const MAIN_BOT_ID = require('../../rsc/config/bot.json').MAIN_ID;
...
class TestBot extends Bot {
    ...
    ready() {
        this.rtm.on('ready', async () => {
            const strFormat =
                '테스트 준비 완료\n' +
                '(테스트 시작 입력시 테스트 시작)';
            await this.rtm.sendMessage(strFormat, PRIVATE_CHANNEL);
        });
    }
    ...
    listen() {
        this.rtm.on('message', async (message) => {
            ...
            // User가 테스트 시작 Case
            else if (text === '테스트 시작' && this.#testing === false) {
    ...
    async* startTest() {
        ...
        await this.rtm.sendMessage(stringFormat, PRIVATE_CHANNEL);
        ...
        await this.rtm.sendMessage(this.#resultBuf, PRIVATE_CHANNEL);
    ...
    async* testGreet() {
        ...
        const send = async (num) => {
            randomStub = sinon.stub(Math, 'random').returns(num / 3);
            randomStub();
            await this.rtm.sendMessage(testData.greeting, PRIVATE_CHANNEL);
        }

        const test = async (expectedGreetText, num) => {
            this.#resultBuf += (greeting() === expectedGreetText) ?
                `|       Test #1${num + 1}  : 성공        |\n` :
                `|       Test #1${num + 1}  : 실패        |\n`;
            randomStub.restore();
        }
    ...
    async testSchedule() {
        ...
        const todaySchedule = scheduleData[Math.floor(Math.random() * Object.keys(scheduleData).length)];

        await this.rtm.sendMessage('학사일정', PRIVATE_CHANNEL)
        await sleep(1500);

        await this.rtm.sendMessage(todaySchedule, PRIVATE_CHANNEL);

        await sleep(1500);

        this.#resultBuf += check(schedule(todaySchedule).success, 2);
    }

    testMenu() {
        const send = new Promise((resolve) => {
            setTimeout(() => {
                resolve(this.rtm.sendMessage(testData.menu, PRIVATE_CHANNEL));
            }, 1000);
        });
    ...
    async testDepartment() {
        ...
        const randDept = department[Math.floor(Math.random() * Object.keys(department).length)];
